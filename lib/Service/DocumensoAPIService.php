...
class DocumensoAPIService {
	...
	public function uploadFile(File $file, array $uploadEndpoint, string $ccUserId): array {
		...
		if ($respCode >= 400) {
			...
			// get team name to build the correct url
			$endPoint = 'api/v1/documents/' . $uploadEndpoint['documentId'];
			$envelope = [];
			$resp = $this->apiRequest($baseUrl, $ccUserId, $endPoint, $envelope, 'GET');
			error_log(json_encode($resp));
			$teamId = 'janepie';
			return [
				'body' => json_decode($body, true),
				'documentUrl' => $baseUrl . 't/' . $teamId . '/documents/' . $uploadEndpoint['documentId'] . '/edit',
			];
		}
	}
